...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
    [MAC_BASE] = LAYOUT_ansi_82(
        ...
        KC_LCTL,  KC_LOPTN, KC_LCMMD,                               KC_SPC,                                 KC_RCMMD, FN_MAC,  CTL_GLOB,  KC_LEFT,  KC_DOWN,  KC_RGHT),
    ...
    switch (keycode) {
        case CTL_GLOB:
            if (record->event.pressed) {
                ...
                register_code(KC_RCTL); // Act as Control immediately on press
            } else {
                unregister_code(KC_RCTL); // Release Control
                
                // If it was a quick tap, send the Globe key
