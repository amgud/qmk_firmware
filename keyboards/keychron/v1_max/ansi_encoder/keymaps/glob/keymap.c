...
// Variable to track the timing of the press
static uint16_t glob_timer;
    ...
    switch (keycode) {
        case LT_GLOB:
            if (record->event.pressed) {
                // Start timer and turn on the Mac Function layer immediately
                glob_timer = timer_read();
                layer_on(MAC_FN);
            } else {
                // Turn off the layer on release
                layer_off(MAC_FN);
                
                // If released before TAPPING_TERM (default 200ms), send the Globe key
                if (timer_elapsed(glob_timer) < TAPPING_TERM) {
                    host_consumer_send(AC_NEXT_KEYBOARD_LAYOUT_SELECT);
                    host_consumer_send(0); // Clear the report immediately
                }
            }
            return false;
    }
